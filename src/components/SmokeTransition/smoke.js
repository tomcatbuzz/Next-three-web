...
import { WebGLRenderer, Scene, Mesh, PlaneGeometry, ShaderMaterial, UniformsUtils, Vector2, Vector3, PerspectiveCamera, ShaderLib  } from 'three';
import vertex from './shaders/cloud.vert';
import fragment from './shaders/cloud.frag';
// import * as from './glsl-fractal-brownian-noise'
import './smoke.module.scss';

/**
 * @param {Number} duration length of animation
 * @returns {AnimationHookReturnValue} pass into <Transition />>
 */
export function useSmoke(duration = 2000, enterDuration = duration / 4) {
    ...
    return {
        ...
        leaveAnimation(el, done) {
            ...
            const { scene, planeMaterial, planeMesh, camera } = setStage({
                gradientSize,
            });
...
function setStage(opts) {
    ...
    const planeMaterial = new ShaderMaterial({
        uniforms: {
            ...
            uColor: { value: new Vector3(.02, .47, .75) },
            uResolution: {
                value: new Vector2(
                    window.innerWidth,
                    window.innerHeight,
...
function handleWindowResize(renderer, camera, planeMesh, planeMaterial) {
    const onWindowResize = () => {
        ...
        planeMesh.scale.set(window.innerWidth / window.innerHeight, 1, 1);
        planeMaterial.uniforms.uResolution.value = new Vector2(
